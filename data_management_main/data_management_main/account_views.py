...
def HOME(request):
    ...
    # Get the bank balance from the first row in the Summary table
    first_summary = Summary.objects.order_by('date').first()
    bank_balance = first_summary.balance if first_summary else 0
...
def ADD_CASHOUT(request):
    ...
    if request.method == "POST":
        ...
        except ExpenseSource.DoesNotExist:
            messages.error(request, 'Expense Source does not exist.')
        ...
        if not projects.exists():
            ...
            messages.warning(request, 'Multiple projects found, using the first one.')
        project = projects.first()

        try:
            cash_out = Decimal(cash_out)
        except Exception:
            messages.error(request, 'Invalid cash out value.')
            return redirect('add_cashout')
