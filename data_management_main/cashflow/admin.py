...
def update_project_summary_on_save(sender, instance, created, **kwargs):
    if created:
        cash_flow = Decimal(instance.cash_in) if isinstance(instance, CashIn) else -Decimal(instance.cash_out)
...
def update_project_summary_on_delete(sender, instance, **kwargs):
    cash_in_transactions = CashIn.objects.filter(date=instance.date, project=instance.project)
    cash_out_transactions = Cashout.objects.filter(date=instance.date, project=instance.project)
...
class ProjectSummaryAdmin(admin.ModelAdmin):
    ...
    # Uncomment if you want to disable delete permission
